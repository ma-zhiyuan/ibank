...
! function(G, $) {
	...
	G.Domain.home = 'http://www.husuli.pw/ibank/';

	// 统一身份认证
	G.Domain.passport = G.Domain.home+"LUser/Navigation/checkLogin";
	
	// 登录
	G.Domain.login = G.Domain.home+"user_login.jsp";
	//个人信息
	G.Domain.info = G.Domain.home+"LUser/User/myinfo";
	//退出
	G.Domain.exit = G.Domain.home+"LUser/Navigation/userExit";

	// 注册
	G.Domain.reg = G.Domain.home + 'views/user/user_registe_a.jsp';

	// 个人中心
	G.Domain.uc = G.Domain.home + "LUser/User/returnMain";

	// 后台管理系统
	G.Domain.admin = G.Domain.home;

	// 帮助中心
	G.Domain.help = G.Domain.home+ 'views/user/user_help.jsp';
...
! function(G, $) {
	...
	var topEle = {
		...
		logined: [
			'<li class="self">',
			'    你好,<a href ="' + Global.Domain.info + '" ><span id="topUser" class="username"></span></a>',
			'    <a class="logout" href="'+Global.Domain.exit+'">退出</a>',
			'</li>',
			'<li>',
			'    <a href="' + Global.Domain.uc + '">我的蓝桥银行</a>',
			'</li>'
		].join(""),

		// 吊顶未登录显示节点
		unlogin: [
			'<li class="self">欢迎使用蓝桥银行服务平台</li>',
			'<li>',
			'    <a href="' + Global.Domain.login + '">登录</a>',
			'    /',
			'    <a href="' + Global.Domain.reg + '">注册</a>',
			'</li>'
		].join("")
	...
	G.Header.init = function() {
		 ...
		 **/
		// 加载当前登录用户的服务器地址
		var loadUrl = Global.Domain.passport;
...
! function(G, $) {
	...
	var bottomEle = [
		...
		'		<li><a href="'+G.Domain.home+'customer_login.jsp" target="_blank">客服系统</a></li>',
		'		<li><a href="'+G.Domain.home+'admin_login.jsp" target="_blank">管理系统</a></li>',
		'		<li><a href="'+G.Domain.home+'views/user/user_help.jsp" target="_blank">帮助中心</a></li>',
